...
WindowSectionsModel::WindowSectionsModel(GuiApplication& app, slint::ComponentHandle<ui::AppWindow> win,
                                         QObject* parent) noexcept
  : QObject(parent),
    mApp(app), mWindow(win),
...
std::optional<ui::WindowSection> WindowSectionsModel::row_data(
    std::size_t i) const {
  return (i < mItems.size()) ? std::optional(mItems.at(i).uiData)
                             : std::nullopt;
}
...
void MainWindow::board3dItemClicked(int section, int tab) noexcept {
  /*if (section >= mSections->row_count()) return;
...
void MainWindow::addTab(ui::TabType type, const QString& title,
  ...
  if (mSections.count() < 2) {
    ...
    mSectionsData->push_back(ui::WindowSection{
